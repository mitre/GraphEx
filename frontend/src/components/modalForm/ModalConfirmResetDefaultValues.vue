	...
	<ModalComponent :title="props.title" @close="onClose">
		<template v-slot:contents>
			<div class="modal-form-contents-container custom-scrollbar">
				<CompositeResetValueEntry :values="props.content" :level="0" />
			</div>
		</template>
...
<script setup lang="ts">
	...
	import CompositeResetValueEntry from '@/components/modalForm/CompositeResetValueEntry.vue';
	import type { CompositeValue } from '@/graph';
	import { defineEmits, defineProps } from 'vue';

	export interface ModalConfirmResetDefaultValuesProps {
		title: string;
		content: CompositeValue[];
		buttons: ButtonProps[];
	}

	const props = defineProps<ModalConfirmResetDefaultValuesProps>();
